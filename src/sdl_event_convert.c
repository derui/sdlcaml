...
SDL_Event* caml_convert_m2c_active_event(value ml_event) {
  ...
  value state_list = Field(ml_event, 1);
...
SDL_Event* caml_convert_event_m2c(value ml_event) {
  ...
  if (Is_long(ml_event)) {
    switch (Int_val(ml_event)) {
      ...
      case MLTAG_USER: event   = caml_convert_m2c_user_event(ml_event); break;
      case MLTAG_SYSWM: event  = caml_convert_m2c_syswm_event(ml_event); break;
    }
  } else {
